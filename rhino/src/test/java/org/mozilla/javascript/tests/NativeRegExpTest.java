...
public class NativeRegExpTest {
    ...
    // tests that rhino lets /x and /u be treated as a literal if it finds any non-hex chars after
    // the x or u
    @Test
    public void testInvalidSingleDigitHexEscape() {
        final String script = "'x1'.match(/\\x1/)[0] + 'u1'.match(/\\u1/)[0];";
    ...
    public void testUnicodeEscapesInClass() {
        //        Utils.assertWithAllModes_ES6("high-low surrogate pair", "😀",
        // "'😀'.match(/[\\uD83D\\uDE00]/u)[0]");
        Utils.assertWithAllModes_ES6(
